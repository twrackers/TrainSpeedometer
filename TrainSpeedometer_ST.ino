...
// Define two RangeWindow objects.
...
void loop() {
  ...
  if (meter.update()) {
    ...
    bool use_metric = digitalRead(METRIC_PIN) == HIGH;
    meter.setMetric(use_metric);
    ...
    if (meter.isUpdated()) {
      ...
      char str[9];
      char* units = use_metric ? "KPH" : "MPH";
      double speed = meter.getSpeed();
      sprintf(str, "%4d%s", (int) round(speed), units);
      display.print(str);
#if LOGGING
#if STREAMING
      Serial << speed << " " << String(units) << endl;
#else
      Serial.print(speed);
      Serial.print(" ");
